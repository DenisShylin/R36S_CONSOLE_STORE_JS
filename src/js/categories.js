...
export function initCategories() {
  ...
  if (videoElement) {
    ...
    // Добавляем стили непосредственно к элементу, чтобы гарантировать отображение
    ...
    videoElement.addEventListener('loadedmetadata', () => {
      try {
        ...
        // Показываем видео и убеждаемся, что оно видимо
        videoElement.style.display = 'block';
        videoElement.style.visibility = 'visible';
        videoElement.style.opacity = '1';
        ...
        // Не пытаемся автоматически воспроизвести здесь
        // Это будет сделано с задержкой ниже
      } catch (err) {
        console.error('Ошибка при обработке метаданных видео:', err);
      }
    });

    // Добавляем задержку перед автовоспроизведением
    setTimeout(() => {
